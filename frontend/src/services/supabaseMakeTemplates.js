...
export async function fetchMakeTemplates() {
  ...
  const res = await fetch(
    `${url.replace(/\/$/, '')}/rest/v1/${TABLE}?select=id,title,description,label,youtube_url,preview_image_url,blueprint_file_url,blueprint_filename,created_at&order=created_at.desc`,
    {
      headers: {
        apikey: key,
