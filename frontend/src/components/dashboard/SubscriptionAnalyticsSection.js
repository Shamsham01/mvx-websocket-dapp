...
/**
 * Webhook & subscription analytics backed by the MakeX API. Rendered below the subscription list.
 */
export default function SubscriptionAnalyticsSection({ user }) {
  ...
  const fetchDashboard = React.useCallback(() => {
    ...
    authAPI
      ...
      .catch(() => setError('Unable to load subscription analytics. Please reconnect and try again.'));
  }, []);
  ...
  return (
    <Box sx={{ mt: 4 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2.5 }}>
        Charts are fed by your MakeX backend. Filter the timeline by clicking a subscription bar.
      </Typography>
      ...
      {error && (
        <Box sx={{ mt: 2.5 }}>
          <ErrorState title="Analytics unavailable" message={error} onRetry={fetchDashboard} />
        </Box>
      )}
      ...
      <SectionCard title="Wallet" sx={{ mt: 3 }}>
        <Stack spacing={1.2}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
            ...
            <Button component={Link} to="/api-docs" variant="outlined" size="small">
              API Docs
            </Button>
          </Stack>
          <Typography variant="body2" color="text.secondary">
            {selectedSubscription
              ...
              : 'Use the table above to manage webhook endpoints; usage fees for MakeX are tracked on the Dashboard.'}
          </Typography>
        </Stack>
      </SectionCard>
    </Box>
